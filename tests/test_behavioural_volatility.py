#!/usr/bin/env python3
"""
Test and validation script for Behavioural Volatility Score feature engineering.

This script validates the correctness of the behavioural volatility score implementation
...
from source_code_package.features.behavioural_volatility_features import (
...
def validate_real_data():
    """Validate the behavioural volatility score on real data."""
    print("\nValidating on real data...")
    
    # Load the generated features
    try:
        df = pd.read_csv('data/processed_data/behavioural_volatility_features.csv')
        print(f"  Loaded {len(df)} records")
        
        # Check for any invalid values
        features = ['FINANCIAL_VOLATILITY', 'ACTIVITY_VOLATILITY', 'EXPLORATION_VOLATILITY', 
                   'BEHAVIOURAL_VOLATILITY_SCORE_RAW', 'BEHAVIOURAL_VOLATILITY_SCORE']
        ...
        max_diff = np.abs(df['BEHAVIOURAL_VOLATILITY_SCORE_RAW'] - expected_raw).max()
        print(f"  Max difference in raw score calculation: {max_diff:.10f}")
        
        # Check normalization
        norm_min = df['BEHAVIOURAL_VOLATILITY_SCORE'].min()
        norm_max = df['BEHAVIOURAL_VOLATILITY_SCORE'].max()
        print(f"  Normalized score range: [{norm_min:.6f}, {norm_max:.6f}]")
        
        print("  ✅ Real data validation passed!")
        
    except FileNotFoundError:
        print("  ⚠️  Behavioural volatility features file not found. Run the main script first.")


def main():
    """Run all tests."""
    print("="*80)
    print("BEHAVIOURAL VOLATILITY SCORE VALIDATION TESTS")
    print("="*80)
