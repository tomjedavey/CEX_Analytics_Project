...
bridge_outflow AS (
    SELECT
        t.origin_from_address AS wallet,
        COUNT(*) AS bridge_outflow_count,
        SUM(t.amount_usd) AS bridge_outflow_volume_usd
    FROM polygon_onchain_core_data.core.ez_token_transfers t
    INNER JOIN polygon_onchain_core_data.core.dim_labels l 
        ON t.origin_to_address = l.address
    ...
    GROUP BY t.origin_from_address
),

bridge_inflow AS (
    SELECT
        t.origin_to_address AS wallet,
        COUNT(*) AS bridge_inflow_count,
        SUM(t.amount_usd) AS bridge_inflow_volume_usd
    FROM polygon_onchain_core_data.core.ez_token_transfers t
    INNER JOIN polygon_onchain_core_data.core.dim_labels l 
        ON t.origin_from_address = l.address
    WHERE l.label_type = 'bridge'
      AND t.block_timestamp >= DATEADD(MONTH, -6, CURRENT_DATE())
      AND t.origin_to_address IN (SELECT wallet FROM sampled_wallets)
    GROUP BY t.origin_to_address
),
...
wallet_features AS (
    SELECT 
        ...
        COALESCE(outflow.bridge_outflow_count, 0) AS bridge_outflow_count,
        COALESCE(inflow.bridge_inflow_count, 0) AS bridge_inflow_count,
        COALESCE(outflow.bridge_outflow_volume_usd, 0) + COALESCE(inflow.bridge_inflow_volume_usd, 0) AS bridge_total_volume_usd
    ...
    LEFT JOIN bridge_outflow outflow ON tx.wallet = outflow.wallet
    LEFT JOIN bridge_inflow inflow ON tx.wallet = inflow.wallet
)
