#!/usr/bin/env python3
"""
Execution script for Behavioural Volatility Score Feature Engineering.

This script serves as a simple execution wrapper around the core functionality
in source_code_package.features.behavioural_volatility_features module.

The script calculates BEHAVIOURAL_VOLATILITY_SCORE based on three main components:
...
The behavioural volatility score measures the inconsistency and unpredictability 
of wallet behaviour across financial, activity, and exploration dimensions.
...
try:
    from source_code_package.features.behavioural_volatility_features import behavioural_volatility_pipeline
...
def main():
    """Main function to execute behavioural volatility score feature engineering."""
    parser = argparse.ArgumentParser(
        description="Calculate BEHAVIOURAL_VOLATILITY_SCORE for crypto wallet behavioural pattern analysis",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python scripts/behavioural_volatility_feature_engineering.py
  python scripts/behavioural_volatility_feature_engineering.py --input data/raw_data/custom_data.csv
  python scripts/behavioural_volatility_feature_engineering.py --output data/processed_data/custom_output.csv
  python scripts/behavioural_volatility_feature_engineering.py --config source_code_package/config/custom_config.yaml
...
Component Details:
  ...
  Exploration Volatility (25%):
    ...
    parser.add_argument(
        '--config', '-c',
        type=str,
        help='Path to configuration YAML file (default: source_code_package/config/config_behavioural_volatility.yaml)'
    )
    ...
    if args.verbose:
        print("Starting Behavioural Volatility Score Feature Engineering...")
    ...
    try:
        ...
        if args.verbose:
            ...
            print(f"Generated behavioural volatility features:")
            print(f"  - FINANCIAL_VOLATILITY")
            print(f"  - ACTIVITY_VOLATILITY") 
            print(f"  - EXPLORATION_VOLATILITY")
            print(f"  - BEHAVIOURAL_VOLATILITY_SCORE_RAW")
            print(f"  - BEHAVIOURAL_VOLATILITY_SCORE")
        ...
        print("\n✅ Behavioural Volatility Score feature engineering completed successfully!")
