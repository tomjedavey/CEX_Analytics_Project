...
def plot_stable_high_value_traders_analytic_score_distributions(
	...
	if "activity_cluster_label" in df.columns:
		...
		fig_filtered.update_layout(
			title="Stable High-Value Transactors: Distribution Across Activity-Based Clusters",
			xaxis_title="Cluster Label",
			yaxis_title="Wallet Count",
			showlegend=False
		)
		if save_dir:
			fig_filtered.write_html(os.path.join(save_dir, "stable_high_value_traders_filtered_cluster_distribution.html"))
		if show:
			fig_filtered.show()
		cluster_figs.append(fig_filtered)
...
if __name__ == "__main__":
	...
	# Plot overlays and cluster analysis for Stable High-Value Transactors
