#!/usr/bin/env python3
"""
Behavioral Volatility Score Feature Engineering Module

This module provides functionality to calculate behavioral volatility scores for cryptocurrency 
...
The behavioral volatility score measures the inconsistency and unpredictability of wallet behavior
...
def load_config(config_path: Optional[str] = None) -> Dict:
    ...
    if config_path is None:
        ...
        config_path = os.path.join(current_dir, '..', 'config', 'config_behavioral_volatility.yaml')
...
def calculate_behavioral_volatility_score(df: pd.DataFrame, config: Dict) -> pd.DataFrame:
    """
    Calculate the complete behavioral volatility score.
    ...
    --------
    pd.DataFrame
        Dataframe with behavioral volatility components and final score
    """
    df_result = df.copy()
    ...
    print("Calculating Behavioral Volatility Score components...")
...
def behavioral_volatility_pipeline(input_path: Optional[str] = None,
                                 output_path: Optional[str] = None,
                                 config_path: Optional[str] = None) -> Tuple[pd.DataFrame, Dict]:
    """
    Complete pipeline for behavioral volatility score calculation.
    ...
    # Calculate behavioral volatility score
    df_result = calculate_behavioral_volatility_score(df, config)
    ...
    print("\n" + "="*80)
    print("BEHAVIORAL VOLATILITY SCORE SUMMARY")
    print("="*80)
    
    print(f"Dataset shape: {df_result.shape}")
    print(f"Behavioral Volatility Score statistics:")
    print(f"  Mean: {df_result['BEHAVIORAL_VOLATILITY_SCORE'].mean():.6f}")
    print(f"  Median: {df_result['BEHAVIORAL_VOLATILITY_SCORE'].median():.6f}")
    print(f"  Std: {df_result['BEHAVIORAL_VOLATILITY_SCORE'].std():.6f}")
    print(f"  Min: {df_result['BEHAVIORAL_VOLATILITY_SCORE'].min():.6f}")
    print(f"  Max: {df_result['BEHAVIORAL_VOLATILITY_SCORE'].max():.6f}")
