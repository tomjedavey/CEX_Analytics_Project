"""
Interaction Mode Features with Activity Thresholds
...
def load_dataset_and_clusters(dataset_name: str, base_data_path: str, clusters_path: str) -> pd.DataFrame:
    ...
    # Dynamically load the appropriate base dataset based on dataset name
    if dataset_name == 'main':
        ...
        cluster_num = dataset_name.replace('cluster_', '')
        cluster_path = f'data/processed_data/cluster_datasets/new_raw_data_polygon_cluster_{cluster_num}.csv'
        if not os.path.exists(cluster_path):
            raise ValueError(f"Cluster dataset not found: {cluster_path}")
...
def calculate_median_feature_values_for_clusters(
    results_dir: str = "data/processed_data/interaction_mode_results",
    min_activity_threshold: float = 0.3,  # Require 30% activity minimum
    min_cluster_size: int = 50,
    output_path: str = "data/processed_data/interaction_mode_results/interaction_mode_cluster_selections.yaml"
) -> Dict[str, Any]:
    """
    Calculate median feature values for clusters, requiring a minimum activity threshold.
    ...
    results['summary'] = generate_summary_statistics(results, target_features)
    ...
    # Print summary
    print_cluster_selection_summary(results)
    return results

def generate_summary_statistics(results: Dict[str, Any], target_features: List[str]) -> Dict[str, Any]:
    """Generate summary statistics for cluster selection."""
    summary = {
...
def print_cluster_selection_summary(results: Dict[str, Any]) -> None:
    """Print summary of cluster selection results."""
    print(f"\n{'='*80}")
    print("CLUSTER SELECTION SUMMARY")
    print(f"{'='*80}")
    print(f"🎯 Algorithm: {results['selection_parameters']['algorithm']}")
